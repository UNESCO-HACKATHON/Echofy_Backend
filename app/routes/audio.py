...
async def analyze_audio(audio_file: UploadFile):
    """
    Accepts an audio file, transcribes it, and runs the text analysis pipeline.
    Returns the same output as the text endpoint.
    """
    try:
        # 1. Transcribe audio
        import tempfile, os
        ...
        transcribed_text = audio.transcribe_audio(audio_file_path)
        os.remove(audio_file_path)

        # 2. Ingest and Parse Text
        parsed_text = ingest_and_parse(transcribed_text)
